...
module.exports = {
  ...
  runIfMain: (problemModule, input, part1, part2) => {
    if (require.main === problemModule) {
      console.log(`Part 1${part1.desc ? ` (${part1.desc})` : ''}:`, part1(input));
        ...
        [part1, part2].forEach((part, index) => {
          ...
          part.tests.forEach((resultOrTest, index) => {
            if (Array.isArray(resultOrTest)) {
              part.tests[index] = {
                input: resultOrTest[0],
                result: resultOrTest[1],
              };
          ...
          if (latest && answers.length && answers.reduce((a, b) => a && b)) {
            ...
            const answer = assignment[`part${index + 1}`](assignment.input);
            test(`\x1b[38;2;255;255;255m${`Personal answer for part ${index + 1}: ${answer}`}\x1b[0m`, () => {});
          }
        });
      });
    });
